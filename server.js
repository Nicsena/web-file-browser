  ...
  try {
    ...
    let list = []
      ...
      items.forEach(item => {
        ...
        const json = {"name": item.name, attributes: { mode: (iteminfo.mode & parseInt('777', 8)).toString(8), mode_bits: iteminfo.mode, size: formatBytes(iteminfo.size), is_file: iteminfo.isFile(), is_symlink: iteminfo.isSymbolicLink(), mimetype: "", created_at: iteminfo.birthtime, modified_at: iteminfo.mtime } }
...
app.get('/files', async (req, res) => {
  ...
  if(list === "ENOENT") return res.status(404).send("Error 404")
  if(list === "EACCES") return res.status(403).send("Error 403")
...
app.get('/files/content', async (req, res) => {
  ...
  if(content === "ENOENT") return res.status(404).send("Error 404")
  if(content === "EACCES") return res.status(403).send("Error 403")
